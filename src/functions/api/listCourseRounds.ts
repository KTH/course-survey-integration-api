...
export default async function handler<T extends APICourseRoundList>(request: HttpRequest, context: InvocationContext, db: Database): Promise<HttpResponseInit> {
  ...
  const selectionStartDate = new Date(Date.now() - (DAY * 270)).toISOString().slice(0,10);
  ...
  let total;
  try {
    total = await db.countByPropertyQuery('endDate', { $lt: selectionEndDate, $gt: selectionStartDate}, 'CourseRound');
